...
def get_bases(pub_grad, num_bases, power_iter=1, logging=False):
    ...
    for i in range(power_iter):
        R = torch.matmul(pub_grad, L)  # dimension: n x k
        L = torch.matmul(pub_grad.T, R)  # dimension: p x k
        orthogonalize(L)
...
class GEP(nn.Module):
    ...
    def get_gradient_embeddings(self, target_grad, logging=False):
        with torch.no_grad():
            num_param_list = self.num_param_list
            num_anchor_grads = target_grad.shape[0]
            ...
            for i, num_param in enumerate(num_param_list):
                ...
                selected_bases, num_bases, pub_error = get_bases(grad, num_bases, self.power_iter, logging)
    ...
    def forward(self, target_grad, logging=False):
        with torch.no_grad():
            ...
            offset = 0
            self.get_gradient_embeddings(target_grad, logging)
            if (logging):
                print('group wise approx error')

            # for i, num_param in enumerate(num_param_list):
            #     # grad = target_grad[:, offset:offset+num_param]
            #     embedding = self.selected_bases_list[i]
            #     # embedding = torch.matmul(grad, selected_bases)
            #     num_bases = self.num_bases_list[i]
            #     # if (logging):
            #     #     cur_approx = embedding  # torch.matmul(torch.mean(embedding, dim=0).view(1, -1), selected_bases.T).view(-1)
            #     #     cur_target = torch.mean(embedding, dim=0)
            ...
            concatnated_embedding = torch.cat(self.selected_bases_list, dim=1)
            ...
            # residual_gradients = target_grad - concatnated_embedding#no_reduction_approx
            ...
            return avg_clipped_embedding.view(-1), avg_target_grad.view(-1)#, avg_clipped_residual_gradients.view(-1), avg_target_grad.view(-1)
